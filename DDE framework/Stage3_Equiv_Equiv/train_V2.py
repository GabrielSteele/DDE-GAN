...
def train (train_dataloader,epoch):
    ...
    for i, (PET,CT) in enumerate(train_dataloader):  # inner loop within one epoch
        ...
        degree = random.randint(1, 179) # Generate random number between 1 and 359
        rot_CT = transforms.functional.rotate(CT, angle=degree)
        rot_PET = transforms.functional.rotate(PET, angle=degree) # Tg(xs)


        #### PET TO CT
        img_model_CT_PET.eval()
        img_model_PET_CT.train()
        fake_ct_img = img_model_PET_CT(PET)  # Git(Tg(xs))
        fake_PET = img_model_CT_PET(fake_ct_img)

        rot_fake_ct_img = img_model_PET_CT(rot_PET) # Git(Tg(xs))
        rot_fake_ct_sin = FP(rot_fake_ct_img).type(torch.FloatTensor).to(device) # F(Git(Tg(xs)))
        rot_fake_pet_sin = sin_model_CT_PET(rot_fake_ct_sin) # Gas(F(Git(Tg(xs))))
        rot_fake_pet_img = BP(rot_fake_pet_sin).type(torch.FloatTensor).to(device) # F-1(Gas(F(Git(Tg(xs)))))

        PET_CT_loss1 = l1_loss(CT,fake_ct_img) # Order matters, (1,2) penalise the loss between 1 and 2
        PET_CT_loss2 = l1_loss(fake_PET,PET)
        PET_CT_loss3 = l1_loss(rot_fake_pet_img,rot_PET)
        PET_CT_loss = PET_CT_loss1 + opt.lamada_PET[0] * PET_CT_loss2 + opt.lamada_PET[1] * PET_CT_loss3

        #PET_CT_loss1 = nn.L1Loss()(CT,fake_CT)  #image_loss_supervise Intra domain consistency
        #PET_CT_loss2 = nn.L1Loss()(fake_PET,PET)  # image_loss_self Intra domain consistency
        #PET_CT_loss3 = nn.L1Loss()(fake_PET_sin,PET)  #sin_loss_self
        #PET_CT_loss = PET_CT_loss1+opt.lamada_PET[0]*PET_CT_loss2+rot_loss_val        #opt.lamada_PET[1]*PET_CT_loss3 # Cycle consistency
        ...
        fake_pet_img = img_model_CT_PET(CT)
        rot_fake_pet_img = img_model_CT_PET(rot_CT) # Gis(Tg(xs))
        rot_fake_pet_sin = FP(rot_fake_pet_img).type(torch.FloatTensor).to(device) # F(Gis(Tg(xs)))
        fake_ct_sin = sin_model_PET_CT(rot_fake_pet_sin) # Gat(F(Gis(Tg(xs))))
        rot_fake_ct_img = BP(fake_ct_sin).type(torch.FloatTensor).to(device) # F-1(Gat(F(Gis(Tg(xs)))))
        fake_CT = img_model_PET_CT(rot_fake_pet_img)

        CT_PET_loss1 = l1_loss(PET,fake_pet_img)
        CT_PET_loss2 = l1_loss(fake_CT,CT)
        CT_PET_loss3 = l1_loss(rot_fake_ct_img, rot_CT)
        CT_PET_loss = CT_PET_loss1 + opt.lamada_CT[0] * CT_PET_loss2 + opt.lamada_CT[1] * CT_PET_loss3

        #CT_PET_loss1 = nn.L1Loss()(PET,fake_PET)
        #CT_PET_loss2 = nn.L1Loss()(fake_CT,CT)
        #CT_PET_loss3 = nn.L1Loss()(fake_CT_sin,CT)
        #CT_PET_loss = CT_PET_loss1+opt.lamada_CT[0]*CT_PET_loss2+rot_loss_val     #opt.lamada_CT[1]*CT_PET_loss3 # cycle consistency
        ...
        # Could expand on this section to include information about how the equivarience has performed
...
if __name__ == '__main__':
    ...
    ckpt = torch.load('./pretrained_model/ct_pet/best_model_sin.pth',map_location=device)
    ...
    ckpt = torch.load('./pretrained_model/pet_ct/best_model_sin.pth',map_location=device)
    ...
    ckpt = torch.load('./pretrained_model/ct_pet/best_model_img.pth',map_location=device)
    ...
    ckpt = torch.load('./pretrained_model/pet_ct/best_model_img.pth',map_location=device)
