...
def train (train_dataloader,epoch):
    ...
    for i, (PET,CT) in enumerate(train_dataloader):  # inner loop within one epoch
        PET = PET.to(device).type(torch.FloatTensor).detach().to(device) # Move the tensors to the gpu for processing
        CT = CT.to(device).type(torch.FloatTensor).detach().to(device)

        fake_PET = model(CT) # Puts the PET data tensor into the model to generate 'fake_ct'
        loss = l1_loss(PET,fake_PET) # Calculate the intra-domain loss
        ...
        Loss.update(loss.item(),1) # Calls the class 'LossAverage' to update parameters for the Loss.item which is part of pytorch
...
def val(val_dataloader):
    ...
    for i, (PET, CT) in enumerate(val_dataloader):  # inner loop within one epoch
        ...
        PET = (PET).type(torch.FloatTensor).detach().to(device)
        CT = (CT).type(torch.FloatTensor).detach().to(device)
