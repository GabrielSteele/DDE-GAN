...
def train (train_dataloader,epoch):
    ...
    for i, (PET, CT) in enumerate(train_dataloader):  # inner loop within one epoch
        ...
        degree = random.randint(1, 179)  # Generate random number between 1 and 359 Should be 0-179 because of projection op
        degree = torch.tensor(degree, dtype=torch.float64).to(device)
        rot_PET = dgm.geometry.transform.rotate(PET, degree)  # Tg(xs)
        rot_PET_sin = FP(rot_PET).type(torch.FloatTensor).to(device)

        ####image_space
        sin_model.eval()
        img_model.train()
        fake_CT = img_model(PET)
        rot_fake_CT = img_model(rot_PET)
        PET_CT_sin = sin_model(rot_PET_sin)
        PET_CT = BP(PET_CT_sin).type(torch.FloatTensor)
        PET_CT = PET_CT.detach().to(device)

        img_loss1 = l1_loss(CT, fake_CT)  # image_loss
        img_loss2 = l1_loss(PET_CT, rot_fake_CT)  # real_sin_loss
        ...
        rot_fake_CT_sin = sin_model(rot_PET_sin)
        PET_CT_img = img_model(rot_PET)
        PET_CT = FP(PET_CT_img).type(torch.FloatTensor)
        PET_CT = PET_CT.detach().to(device)

        sin_loss1 = l1_loss(CT_sin.unsqueeze(1), fake_CT_sin)
        sin_loss2 = l1_loss(PET_CT, rot_fake_CT_sin)
...
if __name__ == '__main__':
    ...
    ckpt = torch.load('./pretrained_model/PETsin-CTsin/best_model.pth',map_location=device)
    sin_model.load_state_dict(ckpt['model'])

    img_model = ResUNet(1,1).to(device)
    ckpt = torch.load('./pretrained_model/PETimg-CTimg/best_model.pth',map_location=device)
