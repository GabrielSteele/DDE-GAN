...
def train (train_dataloader,epoch):
    ...
    for i, (PET,CT) in enumerate(train_dataloader):  # inner loop within one epoch
        ...
        rot_CT_sin = FP(rot_CT).type(torch.FloatTensor).to(device)

        ####image_space
        sin_model.eval()
        img_model.train()
        fake_PET = img_model(CT)
        rot_fake_PET_sin = sin_model(rot_CT_sin)
        rot_fake_PET_sin_img = BP(rot_fake_PET_sin)

        img_loss1 = l1_loss(PET, fake_PET)  # intra-domain consistency image_loss
        img_loss2 = l1_loss(rot_PET, rot_fake_PET_sin_img)  # intra-domain consistency real_sin_loss
        ...
        sin_loss2 = l1_loss(rot_PET_sin, rot_fake_PET_sin)
